...
router.post("/", userAuth, requirePermission("ai_search", "search"), async (req, res) => {
  try {
    ...
    const authorizedResults = results.filter(r => allowedMeetingIds.includes(r.meetingId.toString()));

    // ✅ Debug log
    console.log(`📤 Returning ${authorizedResults.length} authorized results to frontend`);
